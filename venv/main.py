...
class Button:
    ...
    def draw(self, img):
        ...
        cv2.putText(img, self.val, (self.pos[0] + 40, self.pos[1] + 60), cv2.FONT_HERSHEY_PLAIN, 2, (50, 50, 50), 2)
...
buttonList.append(Button((800, 400 + 150), 400, 100, "CLEAR"))
        ...
        if length < 50:
            for i, button in enumerate(buttonList):
                if button.checkClick(x, y) and delayCounter == 0:
                    ...
                    if myValue == "=":
                        myEquation = str(eval(myEquation))[:12]
                    elif myValue == "CLEAR":
                        myEquation = ""
                    else:
                        if len(myEquation) < 12:
                            myEquation += myValue
                    delayCounter = 1
